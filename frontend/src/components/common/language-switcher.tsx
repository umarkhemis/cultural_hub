...
export function LanguageSwitcher() {
  ...
  return (
    <div className="relative flex items-center">
      <Globe className="pointer-events-none absolute left-1.5 h-3 w-3 text-white/60" />
      <select
        value={language}
        onChange={(e) => setLanguage(e.target.value as LanguageCode)}
        className="appearance-none rounded-lg border border-white/20 bg-white/5 pl-6 pr-1 py-1.5 text-xs text-white/80 outline-none hover:bg-white/10 hover:text-white transition-all cursor-pointer backdrop-blur-sm w-14"
