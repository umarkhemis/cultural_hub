// cultural_hub\frontend\src\components\feed\FeedNavbar.tsx
  ...
  return (
    <div className="absolute top-0 left-0 right-0 z-40">
      ...
      <div className="relative flex items-center justify-between px-5 pt-4 pb-3">
        ...
        <div className="pointer-events-auto hidden items-center gap-2 rounded-2xl border border-white/10 bg-black/40 px-3 py-2 backdrop-blur-md sm:flex">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              onClick={(e) => e.stopPropagation()}
              className="flex items-center gap-2 rounded-xl px-3 py-1.5 text-sm font-medium text-white/70 transition-all hover:bg-white/10 hover:text-white"
            >
      ...
      {menuOpen && (
        <div
          className="pointer-events-auto mx-4 mt-2 rounded-2xl border border-white/10 bg-black/80 backdrop-blur-md p-2"
          onClick={(e) => e.stopPropagation()}
        >
          {navLinks.map((link) => (
            <Link
              ...
              className="flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium text-white/80 hover:bg-white/10 hover:text-white transition-all"
            >
          ...
          {!isAuthenticated && (
            <Link
              href={ROUTES.login}
              onClick={() => setMenuOpen(false)}
              className="mt-2 flex items-center justify-center rounded-xl bg-amber-400 px-3 py-3 text-sm font-bold text-slate-900 hover:bg-amber-300 transition-all"
            >
