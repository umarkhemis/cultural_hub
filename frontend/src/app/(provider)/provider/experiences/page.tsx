...
// src/app/(provider)/provider/experiences/page.tsx
"use client";
...
import { AnalyticsDrawer } from "@/src/features/provider/analytic-drawer";

const PAGE_SIZE = 12;

type SortKey = "newest" | "oldest" | "most_liked" | "most_commented" | "most_viewed";
...
export default function ProviderExperiencesPage() {
  ...
  const totalViews = experiences.reduce((s, e) => s + (e.views_count ?? 0), 0);
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      result = result.filter(
        (e) => e.caption.toLowerCase().includes(q) || e.location?.toLowerCase().includes(q)
      );
    }
    ...
    result.sort((a, b) => {
      ...
      if (sort === "most_viewed") return (b.views_count ?? 0) - (a.views_count ?? 0);
      return 0;
    });
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-6xl px-4 pt-20 pb-20 sm:px-6 lg:px-8">
          ...
          <>
            ...
            <div className="mb-6 grid grid-cols-3 gap-4">
              {[
                ...
                { icon: <Eye className="h-4 w-4 text-emerald-400" />, value: totalViews, label: "Views" },
            ...
            <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <select value={status} onChange={(e) => { setStatus(e.target.value as StatusFilter); resetPage(); }} className={selectClass}>
                  <option value="all">All statuses</option>
                  <option value="published">Published</option>
                  <option value="draft">Draft</option>
                  <option value="archived">Archived</option>
                </select>
                <select value={sort} onChange={(e) => { setSort(e.target.value as SortKey); resetPage(); }} className={selectClass}>
                ...
                <div className="flex items-center rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                  <button onClick={() => setView("list")} className={`p-2.5 transition-colors ${view === "list" ? "bg-slate-900 text-white" : "text-slate-400 hover:text-slate-600"}`}>
                    <List className="h-4 w-4" />
                  </button>
                  <button onClick={() => setView("grid")} className={`p-2.5 transition-colors ${view === "grid" ? "bg-slate-900 text-white" : "text-slate-400 hover:text-slate-600"}`}>
            ...
            {selected.size > 0 && (
              <div className="mb-4 flex items-center justify-between rounded-2xl border border-red-200 bg-red-50 px-4 py-3">
                ...
                <button
                  ...
                  className={`flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-bold transition-all disabled:opacity-50 ${bulkConfirm ? "bg-red-600 text-white hover:bg-red-700" : "bg-white border border-red-200 text-red-600 hover:bg-red-100"}`}
                >
            ...
            {filtered.length > 0 && view === "list" && (
              <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div className="flex items-center gap-4 border-b border-slate-100 bg-slate-50 px-4 py-3">
                  <input type="checkbox" checked={selected.size === paginated.length && paginated.length > 0} onChange={toggleSelectAll} className="h-4 w-4 rounded border-slate-300 accent-slate-900" />
                ...
                {paginated.map((item, i) => (
                  <ProviderExperienceRow
                    ...
                    onViewAnalytics={() => setAnalyticsId(item.id)}
                  />
                ))}
              </div>
            )}
            ...
            {totalPages > 1 && (
              <div className="mt-8 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-600 shadow-sm hover:bg-slate-50 disabled:opacity-40 transition-all">
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1)
                      ...
                      .map((p, i) =>
                        p === "..." ? (
                          ...
                          <button key={p} onClick={() => setPage(p as number)} className={`h-8 w-8 rounded-xl text-xs font-semibold transition-all ${page === p ? "bg-slate-900 text-white shadow-sm" : "border border-slate-200 bg-white text-slate-600 hover:bg-slate-50"}`}>
                            {p}
                          </button>
                        )
                      )}
                  </div>
                  <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-600 shadow-sm hover:bg-slate-50 disabled:opacity-40 transition-all">
