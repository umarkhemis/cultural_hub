  ...
  const onSubmit = async (values: SiteDetailsValues) => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/providers/complete-profile`,
        {
          method: "POST",
          headers: {
      ...
      if (!res.ok) {
        ...
        const message =
          body?.message ||
          body?.detail ||
          "Failed to save site details. Please try again.";
  ...
  return (
    <div className="relative min-h-screen w-full">
      <div className="fixed inset-0 z-0">
        ...
        <div className="absolute bottom-0 left-0 right-0 h-48 bg-gradient-to-t from-slate-950/80 to-transparent sm:h-64" />
      </div>
      ...
      <div className="relative z-10 flex min-h-screen flex-col items-center px-3 py-0 sm:px-6">
        ...
        <div className="mb-5 w-full max-w-2xl sm:mb-8">
          <div className="mb-2.5 inline-flex items-center gap-1.5 rounded-full border border-amber-400/20 bg-amber-400/10 px-2.5 py-1 sm:mb-4 sm:gap-2 sm:px-3 sm:py-1.5">
            <Sparkles className="h-3 w-3 text-amber-400 sm:h-3.5 sm:w-3.5" />
            <span className="text-[10px] font-semibold text-amber-400 sm:text-xs">
              One more step
            </span>
          </div>
          ...
          <p className="mt-1 max-w-md text-xs leading-5 text-slate-300 sm:mt-2 sm:text-sm sm:leading-6">
        ...
        >
          ...
          >
            <div className="relative">
              ...
              <textarea
                ...
                rows={3}
                className="w-full resize-none rounded-xl border border-white/25 bg-white/15 py-2.5 pl-9 pr-3 text-xs text-white placeholder-slate-400 outline-none transition-all focus:border-amber-400/60 focus:bg-white/20 focus:ring-2 focus:ring-amber-400/20 sm:rounded-2xl sm:py-3 sm:pl-10 sm:pr-4 sm:text-sm"
          ...
          <button
            ...
            className="group flex w-full items-center justify-center gap-2 rounded-xl bg-amber-400 px-6 py-3 text-sm font-bold text-slate-900 shadow-xl shadow-amber-400/20 transition-all hover:bg-amber-300 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60 sm:rounded-2xl sm:py-4"
          >
            {isSubmitting ? (
              <>
