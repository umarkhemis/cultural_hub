...
// This page is shown to providers who registered via Google OAuth.
// They are already authenticated — they just need to fill in their cultural site info.
"use client";
...
// ---- schema (only site fields, no password needed) ----
...
async function submitSiteDetails(values: SiteDetailsValues): Promise<void> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/provider/site-details`, {
    ...
    credentials: "include", // session cookie from Google OAuth
...
export default function ProviderCompletePage() {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<SiteDetailsValues>({
    resolver: zodResolver(siteDetailsSchema),
    defaultValues: {
      site_name: "", location: "", description: "", contact_email: "", contact_phone: "",
    },
  });
  ...
  return (
    <div className="relative min-h-screen w-full">
      ...
      <div className="fixed inset-0 z-0">
        <img src="/mock/kigezi_mountain.jpg" alt="Cultural landscape" className="h-full w-full object-cover" />
      ...
      <div className="relative z-10 flex min-h-screen flex-col items-center px-3 py-0 sm:px-6">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-2xl space-y-3 pb-8 sm:space-y-5">
          ...
          {/* Contact info */}
          <div className="grid grid-cols-2 gap-2 sm:gap-4">
            <FormField label="Contact Email (optional)" htmlFor="contact_email" error={errors.contact_email?.message}>
              <div className="relative">
            ...
            <FormField label="Contact Phone (optional)" htmlFor="contact_phone" error={errors.contact_phone?.message}>
              <div className="relative">
          ...
          >
            {isSubmitting ? (
              <><span className="h-4 w-4 rounded-full border-2 border-slate-900/30 border-t-slate-900 animate-spin" />Saving...</>
            ) : (
              <>Finish Setup<ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" /></>
            )}
          </button>
        </form>
      </div>
